...
def create_gold_table() -> None:
    """
    Create or replace the Gold.product_metrics table in BigQuery.

    Calculates product sales metrics from Gold.obt and orders results by total quantity sold.
        ...
        bq.run_bq_query(f'''
                ...
                WITH product_metrics AS (
                    SELECT
                        ...
                        SAFE_DIVIDE(
                            SUM(qty), 
                            DATE_DIFF(yesterday, MIN(order_date_created), DAY) + 1
                        ) AS avg_daily_sales,
